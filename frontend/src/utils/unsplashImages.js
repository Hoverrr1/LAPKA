export const UNSPLASH_FALLBACK_IMAGE = 'https://images.unsplash.com/photo-1506744038136-46273834b3fb';
...
export const isUnsplashPhotoUrl = (src = '') =>
  /^https:\/\/images\.unsplash\.com\/photo-/.test(String(src).trim());

export const getUnsplashBase = (src) => {
  const value = String(src || '').trim();
  return isUnsplashPhotoUrl(value) ? value.split('?')[0] : UNSPLASH_FALLBACK_IMAGE;
};

export const isRemoteImageUrl = (src = '') =>
  /^https?:\/\//.test(String(src).trim());

export const isLocalPreviewUrl = (src = '') =>
  /^(blob:|data:image\/)/.test(String(src).trim());

export const buildUnsplashUrl = (src, variant = 'full') => {
  if (isLocalPreviewUrl(src)) return String(src).trim();
  if (isRemoteImageUrl(src) && !isUnsplashPhotoUrl(src)) return String(src).trim();
  const params = IMAGE_PARAMS[variant] || IMAGE_PARAMS.full;
  return `${getUnsplashBase(src)}?w=${params.width}&auto=format&fit=crop&q=${params.quality}`;
};

export const buildUnsplashSrcSet = (src) => {
  if (isLocalPreviewUrl(src)) return undefined;
  if (isRemoteImageUrl(src) && !isUnsplashPhotoUrl(src)) return undefined;
  const base = getUnsplashBase(src);
  return [400, 800, 1200]
    .map((width) => `${base}?w=${width}&auto=format&fit=crop&q=${width === 400 ? 70 : 80} ${width}w`)
    .join(', ');
};

export const getCategoryUnsplashBase = (category = 'Food', seed = '') => {
...
export const getProductImageBase = (product = {}) => {
  if (isUnsplashPhotoUrl(product.image)) return getUnsplashBase(product.image);
  if (isLocalPreviewUrl(product.image)) return product.image;
  if (isRemoteImageUrl(product.image)) return product.image;
  if (isRemoteImageUrl(product.photo?.url)) return product.photo.url;
  return getCategoryUnsplashBase(product.category, product._id || product.name || product.category);
};

export const getProductImageUrl = (product = {}, variant = 'full') =>
  buildUnsplashUrl(getProductImageBase(product), variant);
